...
def features(imgPath1, imgPath2, wlen):
    ...
    for i in range(min(len(des1), len(des2))):
        ...
        if m.distance < ratio_thresh * n.distance:
            print(str(kp1[n.queryIdx].pt))
            print(str(kp2[m.trainIdx].pt) + " " + str(kp2[n.trainIdx].pt))
    ...
    plt.imshow(imgShow1), plt.savefig("fig1.png")
    plt.imshow(imgShow2), plt.savefig("fig2.png")
    print(str(len(des1)) + " " + str(len(des2)) + " " + str(len(knn_matches)) + " " + str(len(good_matches)))
    return good_matches

def flowMap(fts, w, h, squareLen):
    data = []
    ...
    print(img)
    cv2.imwrite('flowMap.png', np.array(img))


def main():
    ft = features("test1.png", "test2.png", 15)
    img = cv2.imread("test1.png")
    flowMap(ft, len(img[0]), len(img), 5)
main()